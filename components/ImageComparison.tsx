...
export function ImageComparison({ 
  ...
  return (
    <div className="fixed inset-0 bg-secondary/80 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-background rounded-xl shadow-lg max-w-4xl w-full mx-4 overflow-hidden">
          ...
          {activeTab === 'compare' ? (
            <>
              ...
              <div className="grid grid-cols-4 gap-4 mb-6 text-sm">
                ...
                <div className="bg-surface p-3 rounded-xl">
                  <div className="text-text-light">Dimensions</div>
                  <div className="font-medium">
                    {dimensions ? `${dimensions.width}×${dimensions.height}px` : 'N/A'}
                  </div>
                </div>
                <div className="bg-surface p-3 rounded-xl">
                  <div className="text-text-light">Taille originale</div>
                  <div className="font-medium">{(originalSize / 1024).toFixed(2)} KB</div>
                </div>
                <div className="bg-surface p-3 rounded-xl">
                  <div className="text-text-light">Réduction</div>
                  <div className="font-medium text-primary">-{reductionPercentage.toFixed(1)}%</div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-8">
              <div>
                ...
                {exifData ? (
                  <div className="space-y-2">
                    {exifData.make && (
                      <div className="grid grid-cols-2">
                        <span className="text-text-light">Appareil</span>
                        <span>{exifData.make} {exifData.model}</span>
                      </div>
                    )}
                    {exifData.exposureTime && (
                      <div className="grid grid-cols-2">
                        <span className="text-text-light">Vitesse</span>
                        <span>{exifData.exposureTime}s</span>
                      </div>
                    )}
                    {exifData.fNumber && (
                      <div className="grid grid-cols-2">
                        <span className="text-text-light">Ouverture</span>
                        <span>f/{exifData.fNumber}</span>
                      </div>
                    )}
                    {exifData.iso && (
                      <div className="grid grid-cols-2">
                        <span className="text-text-light">ISO</span>
                        <span>{exifData.iso}</span>
                      </div>
                    )}
                    {exifData.focalLength && (
                      <div className="grid grid-cols-2">
                        <span className="text-text-light">Focale</span>
                  ...
                  <p className="text-text-light">Aucune métadonnée EXIF disponible</p>
                )}
              </div>
              <div>
                ...
                <div className="space-y-2">
                  <div className="grid grid-cols-2">
                    <span className="text-text-light">Réduction de taille</span>
                    <span className="text-primary">-{reductionPercentage.toFixed(1)}%</span>
                  </div>
                  <div className="grid grid-cols-2">
                    <span className="text-text-light">Format original</span>
                    <span>{name.split('.').pop()?.toUpperCase()}</span>
                  </div>
                  <div className="grid grid-cols-2">
                    <span className="text-text-light">Format optimisé</span>
